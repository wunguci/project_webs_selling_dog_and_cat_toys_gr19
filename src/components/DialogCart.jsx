...
const DialogCart = ({ onClose }) => {
  ...
  return (
    <div 
      className="absolute right-0 w-80 bg-white shadow-2xl rounded-xl p-5 border border-gray-300 z-50"
            ...
            {cartItems.map((item, index) => (
              ...
              >
                ...
                <div className="flex-1">
                  <p className="text-gray-800 text-sm font-medium line-clamp-2">{item.name}</p>
                  <p className="text-brown font-semibold">
                    {item.price.toLocaleString()}đ<span className="text-gray-500 text-xs ml-1">x {item.quantity}</span>
                  </p>
                </div>
                <button 
